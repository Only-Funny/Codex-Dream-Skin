...
const snapshotter = path.join(linuxRoot, "scripts", "snapshot-theme-zip.mjs");
const extractor = path.join(linuxRoot, "scripts", "extract-theme-zip-linux.sh");
let tempRoot;
...
await test("Linux ZIP snapshot and extraction boundaries", {
  skip: process.platform !== "linux" && "requires Linux archive tools",
}, async () => {
  ...
  console.log("PASS: Linux ZIP snapshots and actual expansion are bounded before staged writes.");
  } finally {
