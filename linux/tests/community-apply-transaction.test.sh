...
/bin/mkdir -p "$SCRIPTS"
/bin/cp "$ROOT/scripts/apply-community-theme-linux.sh" "$SCRIPTS/"
/bin/cp "$ROOT/scripts/theme-switch-lock-linux.sh" "$SCRIPTS/"
...
/usr/bin/printf '%s\n' \
  ...
  'fi' \
  > "$SCRIPTS/status-dream-skin-linux.sh"
...
/usr/bin/printf '%s\n' \
  ...
  > "$SCRIPTS/snapshot-active-theme-linux.sh"
...
run_case() {
  ...
  set +e
  HOME="$home" NODE="$NODE" "$SCRIPTS/apply-community-theme-linux.sh" \
    --id imported-theme \
