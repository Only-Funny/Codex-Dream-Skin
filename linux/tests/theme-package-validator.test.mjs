...
import { loadTheme as loadLinuxTheme } from "../scripts/injector.mjs";
import { loadTheme as loadWindowsTheme } from "../../windows/scripts/injector.mjs";

const here = path.dirname(fileURLToPath(import.meta.url));
const linuxRoot = path.resolve(here, "..");
const projectRoot = path.resolve(linuxRoot, "..");
const validator = path.join(linuxRoot, "assets", "theme-package-validator.mjs");
const linuxInjector = path.join(linuxRoot, "scripts", "injector.mjs");
const windowsInjector = path.join(projectRoot, "windows", "scripts", "injector.mjs");
// The shell-importer integration below is macOS-only (signed Codex app and
// ~/Library state), so it keeps the macOS script path and stays darwin-gated.
const importer = path.join(projectRoot, "macos", "scripts", "import-theme-zip-macos.sh");
const fixtureImage = path.join(linuxRoot, "assets", "portal-hero.png");
...
try {
  ...
  const [legacyMac, legacyWindows] = await Promise.all([
    loadLinuxTheme(legacyOfficial.source),
  ...
  for (const [injector, stage] of [[linuxInjector, macos.stage], [windowsInjector, windows.stage]]) {
  ...
  const [loadedMac, loadedWindows] = await Promise.all([
    loadLinuxTheme(macosBoundaries.stage),
  ...
  await assert.rejects(loadLinuxTheme(overlongCopy.source), /invalid brandSubtitle field/);
  ...
  await assert.rejects(loadLinuxTheme(controlCopy.source), /invalid quote field/);
