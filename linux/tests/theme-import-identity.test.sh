...
STATE_ROOT="$TMP/home/.local/state/codex-dream-skin"
...
FALLBACK_STATE_ROOT="$FALLBACK_HOME/.local/state/codex-dream-skin"
...
/usr/bin/env HOME="$FALLBACK_HOME" NODE="$NODE" \
  "$ROOT/scripts/import-theme-zip-linux.sh" --file "$MISSING_ARCHIVE" \
...
/usr/bin/env HOME="$FALLBACK_HOME" NODE="$NODE" \
  "$ROOT/scripts/import-theme-zip-linux.sh" --file "$NON_STRING_ARCHIVE" \
...
if /usr/bin/env HOME="$FALLBACK_HOME" NODE="$NODE" \
  "$ROOT/scripts/import-theme-zip-linux.sh" --file "$INVALID_ARCHIVE" \
