...
record_pause_error() {
  ...
  finish_client_operation "${PORT:-9335}" error "$(dreamskin_text pause_failed)" \
...
# Linux: read one field from the injector's operation-control-ack.json
# (machine-written JSON, one key per line). Handles string and number values.
# Replaces the macOS property-list extractor reads of the same fields.
read_ack_json_field() {
  local key="$1"
  LC_ALL=C /usr/bin/sed -n \
    -e 's/^[[:space:]]*"'"$key"'"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/p' \
    -e 's/^[[:space:]]*"'"$key"'"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\1/p' \
    "$OPERATION_ACK_PATH" 2>/dev/null | /usr/bin/head -n 1
}
...
operation_ack_matches() {
  ...
  acknowledged_token="$(read_ack_json_field operationToken)"
  acknowledged_mode="$(read_ack_json_field mode)"
  acknowledged_pid="$(read_ack_json_field injectorPid)"
...
# macOS dropped any launchd job that would relaunch ChatGPT with CDP after
# quit; Linux launches Codex directly per command, so this hook is a no-op.
