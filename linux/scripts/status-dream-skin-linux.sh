...
# Fast status for the dreamskin menu / status display. No install-verification
# or CDP probes by default.
...
read_json_text_field() {
  # Parse machine-written JSON (one key per line) with sed only, so the fast
  # status path never starts a Node process. Handles "key": "string" and
  # "key": number.
  local text="$1"
  local key="$2"
  [ -n "$text" ] || return 0
...
# macOS read the operation-state property list; the Linux engine writes
# operation-state.json (same status/message/updatedAt fields, one key per
# line), so the shared JSON field reader above is used directly.

# Keep this check deliberately shell/ps-only: status may be invoked every few
# seconds and must not perform install verification, CDP, or Node startup.  A
# live PID alone is not enough because a stale state file can outlive the
# watcher and its PID may later be reused by an unrelated process.
...
if [ -f "$OPERATION_STATE_PATH" ]; then
  ...
  operation_status="$(read_json_text_field "$OPERATION_SNAPSHOT" status)"
  operation_message="$(read_json_text_field "$OPERATION_SNAPSHOT" message)"
  operation_updated_at="$(read_json_text_field "$OPERATION_SNAPSHOT" updatedAt)"
  now="$(/bin/date +%s)"
