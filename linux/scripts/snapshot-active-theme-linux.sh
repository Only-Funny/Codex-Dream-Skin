...
# Provides assert_inherited_theme_switch_lock. The Linux copy is ported with
# the theme pipeline scripts (Task 7) and is functionally identical; the lock
# directory/token protocol is platform-neutral.
...
[ -n "$DESTINATION" ] || fail "Usage: snapshot-active-theme-linux.sh --destination <private-active-before-dir>"
ensure_state_root
