...
[ -n "$ARCHIVE" ] || fail "Usage: import-theme-zip-linux.sh --file <theme.zip>"
...
"$SCRIPT_DIR/extract-theme-zip-linux.sh" "$ARCHIVE_SNAPSHOT" "$EXTRACT_STAGE"
...
# The publisher emits one JSON object consumed by the platform menu. It
