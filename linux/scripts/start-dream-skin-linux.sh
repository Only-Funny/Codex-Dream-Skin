...
# Linux: Codex is launched directly by this script; there is no LaunchServices
# activation step. Kept as a no-op so the macOS start flow maps 1:1.
...
# A connected renderer can show progress before the install check. Before this
# script launches or restarts Codex, verify the complete install. Linux has a
# single verification mode (dpkg integrity + OpenAI repo, or a recorded
# AppImage/binary approval), so the macOS deep/quick split does not apply.
...
# macOS used LaunchServices activation here to reach the already-running,
# identity-bound App. Linux starts Codex directly and needs no activation
# step; the call is kept as a documented no-op (see above).
activate_codex_window
