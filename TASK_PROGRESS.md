...
- [complete] Task 8: `linux/scripts/dreamskin.sh` 主入口（无参交互菜单 1-9/A/D/U/0 + 子命令 dispatch + --self-test-source 可 source）+ `dreamskin-dispatch.test.sh`（TDD）。计划代码自带 3 处坑均已修：bg/import 传参必须是 `--file`（load-image/import 脚本只收 --file，计划写 --image/裸位置参数）、doctor 不传 --doctor（status 脚本拒绝未知参数）、菜单 0/空输入退出。另外自启动 desktop entry 的 Exec 改绝对路径 `/bin/bash "$SCRIPT_DIR/dreamskin.sh" start`（GUI 会话 PATH 不可靠）、theme list 加 NODE 守卫（common-linux.sh 不初始化 NODE）。质量审查实机复现 import bug 后复审 APPROVED。提交 a5245bd + 8ded49b（fix）+ 计划文档订正（e4be4a4 NUL 字节、3cf64a2 代码块）。
- [complete] Task 9: check-update 移植（plutil→node 读 JSON、osascript 弹窗→终端 y/N、open→xdg-open 兜底、--json 输出字节一致、TEST_RESPONSE_FILE 保留）、install 移植（.command launcher 段→.local/bin 软链+desktop entry+xdg-mime，--no-launch 默认保持）、一键换肤三件套（community-http.mjs 受限客户端：零重定向/CL 预检/流式上限/契约字段校验；community-apply.mjs 下载 sha256+精确字节校验；community-apply-linux.sh 编排，事务根在保留命名空间 `.community-apply-`）。质量审查抓出计划级缺陷：desktop entry 缺 %u 且 dreamskin.sh 无 URL 路由→一键换肤主链路不通（已修：`Exec=...dreamskin.sh community %u` + main() dreamskin:// 直派）；拒绝路径不 destroy 连接→涓流服务器可挂死（已修：全部 reject 分支 response.destroy()）；测试文件含字面 NUL 存为 git binary（已修 NUL 转义 + .gitattributes 防御）。附 User-Agent/maxHeaderSize/URL 门精确化/JSON 分块累积/软链覆盖守卫/下载器独立测试（DREAMSKIN_COMMUNITY_API_ORIGIN 覆盖）。复审 APPROVED。提交 ecddf8b + ff57da9 + 1002d2b + 417b7f2（死 .command 清理）+ 计划文档订正 e77da86。
- [complete] Task 10: .deb 打包全套（installer/ 素材：control+postinst+prerm+postrm+desktop；build-deb.sh；deb-content 断言测试；run-tests.sh 扩展 node>=18 + installer 无扩展名脚本 bash -n）。质量审查实测修掉：postrm 无条件 rm -rf /opt 会在每次升级时删掉新装的文件（已加 purge 门）；umask 027/077 下 dpkg-deb 拒收 control 目录、umask 002 下会把可组写 usr/bin 打进包（umask 022 + STAGE/opt 目录 chmod 755 归一化）；deb 断言对软链目标行的盲点（./ 锚定 + 软链类型专断）；nodejs 下限收紧 >=18.0；无 dpkg-deb/rsync 时跳过而非失败。复审 APPROVED。遗留：build-deb.sh:14 printf 一字符 bug 折进 Task 11。提交 7fc4397 + f88d07a（fix）。
- [complete] Task 11: build-tarball.sh（install.sh wrapper + .gitattributes 排除 + 权限归一化 + 内容断言）+ build-release-linux.sh（check-first sync 闸门——先查漂移失败即停、再确定性再生；清理残留包；测试；双包；SHA256SUMS）。runtime/theme-package-validator.mjs 源修复（platform Set + manifest max 2→3 + allowed 加 linux）→ 三端资产重新生成（cmp 一致，Task 7 手补注释被正确取代）；build-deb.sh printf 修复一并落地。质量审查修掉：tarball 曾继承可组写权限（6 dirs 775/17 scripts 775，已归一化全部 644/755）；SHA256SUMS 曾混入旧版本残留包（已前置清理）；sync 闸门原为死代码（先 sync 后 check 永触不发，已重排 check-first 并实测触发）。validator 补 4 个 linux 平台测试。复审 APPROVED。提交 c1dfbb2 + f64265d + bc4b5b3（fix）。
- [complete] Task 12: macos/tests/release-workflow.test.mjs 新增 linux/VERSION 与 macos/VERSION 平价断言块（该文件原本无版本断言，且被 ci/release 的 node --test glob 双双拾起，create-tag 前拦版本漂移）；ci.yml 追加 linux-tests job（ubuntu-24.04 + sync check + 全套件 + release 构建 + artifact 上传，SHA 沿用仓库固定惯例）；release.yml 按实际机制新增 build-linux job 镜像 build-macos/windows 并接入 publish-release 全链路（test -s/sha256sum/release notes/upload/create/双 expected 数组均更新，计数复核 5=5）。质量审查重点排查 SHA256SUMS 合并碰撞（各平台只传各自二进制、checksums 由 publish 统一生成，无碰撞）与版本 env/file 分歧（guard 链 + 平价测试 fail-loud）。复审 APPROVED。提交 9f5de90。
- [carry] Task 14 可选硬化：guard 版本源循环补 linux/VERSION（现为传递性保证）；Fei-Away 链接基址系历史遗留（新行沿用）。Task 13 文档落地后给 release notes 补 install-linux 链接。
